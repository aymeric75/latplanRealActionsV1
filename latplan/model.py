...
It contans only as many rows as the available actions. Unused actions are removed."""
        ...
        print(list_of_binary_actions[:5])
        print(list_of_binary_actions.shape)
        # (24, 1, 6)
    ...
    def dump_actions(self,transitions,**kwargs):
        print("trassss")
        print(transitions[0].shape)
        print("zzzz")
        transitions_z = self.encode(transitions[0],**kwargs)
        print(transitions_z.shape)
...
class DetActionMixin:
    ...
    #     retour = Sequential(self.action_encoder_net)(concatenate([p_pre,p_suc],axis=-1))
    #     print("retour")
    #     print(retour)
    #     # Tensor("gumbel_softmax_1/gumbel_0/cond/Merge:0", shape=(?, 1, 6000), dtype=float32)
    #     return retour


    def _action(self,desc):
        retour = Sequential(Lambda(lambda desc: desc))
        print("retour")
        print(retour)
        # <function Sequential.<locals>.<lambda> at 0x7fab4d1aa4c0>
        return retour
...
class BaseActionMixinAMA4Plus(BidirectionalMixin, BaseActionMixin):
    ...
    def _build_around(self,input_shape):
        print("input_shape in _build_around")
        print(input_shape)
        A  = self.parameters["A"]
        self.p_a_z0_net = [
            Dense(A),
    ...
    def SpecificLoss(self,logit_q,logit_p=None,p=None):
        ...
        if (logit_p is None) and (p is None):
            ...
            loss = K.sum(q * log_q, axis=-1) + K.log(K.cast(6, "float"))
        elif logit_p is not None:
            s = K.shape(logit_p)
            logit_p = wrap(logit_p, K.reshape(logit_p, (s[0], 1, 6)))
            p = K.softmax(logit_p)
    ...
    def _build_primary(self,input_shape):
        ...
        variational_layer = Variational()

        action_input = Input(shape=(6,))
        reshaped_action_input = Reshape((6,))(action_input)

        print(reshaped_action_input) # Tensor("reshape_6/Reshape:0", shape=(?, 6), dtype=float32)
        ...
        # note: _action takes a probability, but feeding 0/1 data in test time is fine (0/1 can be seen as probabilities)
        #action    = self._action (p_pre,p_suc)
        #action = self._action(action_input)
        action = action_input
        ...
        # l_action doit être Tensor("dense_2/BiasAdd:0", shape=(?, 6000), dtype=float32)
        #(l_action,  ), _ = action_input.variational_source # see Variational class
        (l_suc_aae, ), _ = z_suc_aae.variational_source # see Variational class
        ...
        kl_a_z0 = self.SpecificLoss(reshaped_action_input, logit_p=Sequential(self.p_a_z0_net)(z_pre))
        #kl_a_z1 = action_input.loss(l_action, logit_p=Sequential(self.p_a_z1_net)(z_suc))
        kl_a_z1 = self.SpecificLoss(reshaped_action_input, logit_p=Sequential(self.p_a_z1_net)(z_suc))
