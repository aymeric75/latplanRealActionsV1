...
"""
    def __init__(self,path,parameters={}, all_actions=None):
        import subprocess
...
Poor python coders cannot enjoy the cleanness of CLOS :before, :after, :around methods."""
 ...
 This method may be overloaded by the subclass into a specific training method, e.g. GAN training."""
        ...
        def generate_logs(data, data_to):
            ...
            # Assuming data and data_to are structured as previously discussed:
            data_1 = np.array([sublist[0] for sublist in data])
            data_2 = np.array([sublist[1] for sublist in data])

            data_to_1 = np.array([sublist[0] for sublist in data_to])
            data_to_2 = np.array([sublist[1] for sublist in data_to])

            # data_2 = np.expand_dims(data_2, -1)
            # data_to_2 = np.expand_dims(data_to_2, -1)

            # data_2 = data_2.reshape(-1, 1, 6)
            # data_to_2 = data_to_2.reshape(-1, 1, 6)


            formatted_data = [data_1, data_2]
            formatted_data_to = [data_to_1, data_to_2]
            ...
            #exit()
            evals = self.nets[0].evaluate(formatted_data,
                                    data_1,
                                    batch_size=batch_size,
                                    verbose=0)
        ...
        try:
            ...
            for epoch in range(start_epoch,start_epoch+epoch):
                ...
                # print("INDICE CACHE")
                # print(indices_cache[:5])
                # print("LEEEEEEEEEN IN DATA  SHAPE")
                # print(len(train_data[0]))
                # print(len(train_data))
                # print("indice cache 0")
                # print(indices_cache[0])
                ...
                # train_data_cache    = [[ train_data   [indices]  ] for indices in indices_cache ]
                # train_data_to_cache = [[ train_data_to[indices] ] for indices in indices_cache ]

                # train_data_cache Should actually be 
                ...
                for train_subdata_cache, train_subdata_to_cache in zip(train_data_cache, train_data_to_cache):
                    ...
                    action_input_data = np.array([item[1] for item in train_subdata_cache])

                    print(action_input_data.shape)
                    #exit()
                    #action_input_data = np.expand_dims(action_input_data, axis=0)
